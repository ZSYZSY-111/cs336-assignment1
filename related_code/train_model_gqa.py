...
CONFIG = {
    ...
    "weight_decay": 0.01,
    "max_l2_norm": 1.0,
    "checkpoint_dir": Path("/root/autodl-tmp/checkpoints_gqa"),
...
from train_transformer_gqa import TransformerLM
...
def main():
    ...
    log_path = Path(config["log_path"]) if config["log_path"] is not None else config["checkpoint_dir"] / "train_log_gqa.csv"
    ...
    print(f"num_heads: {config['num_heads']} | num_kv_heads: {config['num_kv_heads']} (GQA)")
